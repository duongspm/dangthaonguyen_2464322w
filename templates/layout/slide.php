<div class="slider">
        ...
        <div class="owl-page owl-carousel owl-theme" data-xsm-items="1:0" data-sm-items="1:0" data-md-items="1:0"
            ...
            <div class="slideshow-item" owl-item-animation>
                <a class="slideshow-image" href="<?=$v['link']?>" target="_blank" title="<?=$v['name'.$lang]?>">
                    <?=$func->getImage(['class' => 'lazy w-100', 'sizes' => '1366x615x1', 'upload' => UPLOAD_PHOTO_L, 'image' => $v['photo'], 'alt' => $v['name'.$lang]])?>
                </a>
            </div>
            <?php } ?>
        </div>
    ...
    <?php } ?>
    <?php if(!empty($descslide)){?>
    <div class="info-slider">
        <span class="info__slide--desc">
            <?=$descslide['desc'.$lang]?>
        </span>
        <div class="name_sl">
            <?=$descslide['name'.$lang]?>
